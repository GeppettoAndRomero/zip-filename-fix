# zip-filename-fix — stamped from heic-to-jpg, ENGINE + CONTENT still TODO

Frozen core and the slug surface are done. This tool currently builds as a HEIC clone
at /zip-filename-fix/. Replace the engine layer before it is real:
...
- [ ] `src/i18n/{en,ja,zh,de,es}.ts` — 5 native content sets via the competitor-grounded
      ...
      Also fix the PWA display name in `public/zip-filename-fix/manifest.json` ("HEIC to JPG").
